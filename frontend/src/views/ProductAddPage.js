...
import { deleteProduct, detailsProduct, listCategories, updateProduct } from "../redux-actions/productActions";
...
props.history.push("/product-list");
  ...
  {loading ? (
          ...
          /> 
            ...
            className="form">
               <div style={{marginBottom:"-3rem", textAlign:"center"}} className=""><h2>Product Details</h2></div>
          <div >
...
{uploading &&  <LinearProgress style={{marginTop:"2%",marginBottom:"-1rem"}} /> }
          ...
          <div >
              ...
              <textarea style={{border:"2px solid steelblue", borderRadius:"8px", fontWeight:"bolder"}} type="text" id="description" rows='3' required placeholder="Enter Product Description" value={description} onChange={(e)=> setDescription(e.target.value)}/>
          
          </div>
          <div >
              <label htmlFor="category" >Category</label>
<select required style={{padding:"1rem", borderRadius:"15px", border:"2px solid steelblue",fontWeight:"bolder", outline:"steelblue"}} 
...
{categories.length>0 ? categories.map(category=>(<option style={{padding:"1rem", fontSize:"1.6rem"}}  value={category.name}>{category.name}</option>)
 ...
 {/* <input type="text" id="category" required placeholder="Enter Product Category" value={category} onChange={(e)=> setCategory(e.target.value)}/> */}
          ...
          <div style={{display:"flex",justifyContent:"space-between" ,flexDirection:"row-reverse", marginTop:"1rem"}} >
              ...
              <button onClick={()=>deleteHandler(productId)} style={{width:"40%", backgroundColor:"transparent", borderColor:"transparent", color:"steelblue", boxShadow:"1px 1px 1px 1px steelblue"}} type="button" className="btn">Cancel</button>
